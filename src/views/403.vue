...
<script>
  ...
  export default {
    ...
    created() {
      this.role = JSON.parse(localStorage.getItem('userInfo'))['roles'][0]
    },
    mounted() {
      this.timeChange()
    },
      ...
      timeChange() {
        this.timer = setInterval(() => {
          if (this.jumpTime) {
            ...
            if (this.role === '服务机构') {
              this.$router.push({ path: '/agencyme' })
              this.delOthersVisitedRoutes({ path: '/agencyme' })
              clearInterval(this.timer)
            } else {
